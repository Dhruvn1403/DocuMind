...
import { generateAccessToken, generateRefreshToken, } from '../functions/token.functions.js';
import { getConnection } from '../utils/Connections.js';
import { isEmail, isMobileNumber, } from '../functions/helper.functions.js';
import { getAccountsModel } from '../models/accounts.model.js';
import { hashPassword, verifyPassword, } from '../functions/encrypt.functions.js';
import { getGoogleAuthURL, googleClient, } from '../functions/google.functions.js';
import { getUsersModel } from "../models/users.model.js";
export const signUp = async (req, res, next) => {
    let session;
    try {
        const { email_number, name, password, userType, uniqueCode, } = req.body;
        ...
        if (existingUser) {
            throw ApiError.conflict('User with given email or mobile number already exists');
        }
        let user = null;
        ...
        res
            ...
            .json(ApiResponse.success({ accessToken, refreshToken, userDetails: user }, 'User registered successfully'));
        return;
    }
    catch (error) {
        try {
...
export const signIn = async (req, res, next) => {
    try {
        const { signInWith, email_number, password } = req.body;
        switch (signInWith) {
            case 'google':
                try {
            ...
            case 'credentials':
                ...
                if (user.status == 'deleted') {
                    throw ApiError.unauthorized('Your account is deleted contact admin to recover.');
                }
...
export const googleAuthCallback = async (req, res, next) => {
    ...
    try {
        ...
        if (!tokens)
            throw ApiError.badRequest('Failed to exchange token');
        googleClient.setCredentials(tokens);
        const { data: google } = await axios.get('https://www.googleapis.com/oauth2/v2/userinfo', {
...
export const googleLoginWithIdToken = async (req, res, next) => {
    let session;
    try {
        ...
        if (user) {
            await Account.updateOne({ userId: user._id, provider: 'google' }, {
                $set: {
                    provider: 'google',
                    idToken: googleId,
                    updatedAt: new Date(),

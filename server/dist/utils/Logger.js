...
const consoleFormat = combine(colorize(), errors({ stack: true }), timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), printf(({ level, message, timestamp, stack, uniqueCode = "Master DB", ...meta }) => {
...
const fileDbFormat = combine(errors({ stack: true }), timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), json());
const transports = [
...
export const createLogger = (uniqueCode) => {
